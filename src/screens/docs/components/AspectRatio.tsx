...
export function AspectRatioDoc() {
  return (
    <div className="flex flex-col gap-10">
      <DocHeader title="AspectRatio">
        子要素を固定比率の box に閉じ込める単純 wrapper。 CSS の aspect-ratio
        property を ratio prop で受けるだけ。 中身は relative box の中に絶対配置
        / 普通の flow どちらも置ける。
      </DocHeader>

      <DocSection
        title="Usage"
        description="ratio に幅÷高さを渡す。 16:9 なら 16/9、 正方形なら 1。"
      >
        <Example
  ...
  <div className="flex h-full items-center justify-center">16:9</div>
      ...
      <DocSection
        title="Common ratios"
        description="grid で並べると同じ幅の中で高さだけ変わるのが見やすい。"
      >
        <Example
  ...
  <AspectRatio key={r} ratio={r}>...</AspectRatio>
      ...
      <DocSection
        title="Image / video frame"
        description="thumbnail を中に置き、 absolute で overlay を重ねる典型例。 box は relative なので absolute 配置がそのまま使える。"
      >
        <Example
