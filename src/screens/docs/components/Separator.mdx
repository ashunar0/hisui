...
import { DocHeader, DocSection } from "@/screens/docs/parts/doc-page";
...
<DocSection title="Horizontal" description="default。 縦に積んだ block の間に挟む。">
  <Example
    code={`<div>
  <p>Section 1</p>
  <Separator />
...
<DocSection title="Vertical" description="orientation=vertical で 1px の縦線。 flex-row の inline 要素間に。">
  <Example
...
<DocSection title="In card" description="header と body、 body と footer の区切り。 card 内で頻出。">
  <Example
...
<DocSection title="Custom color" description="className で bg-* を上書き。 muted な区切りや brand 色強調に。">
  <Example
